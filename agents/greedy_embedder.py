...
class GreedyEmbedder(BaseEmbedder):
    """
    Greedy Embedder:
    Selects substrate nodes with the most available resources
    (e.g., CPU) for each virtual node in descending order.
    """
    ...
    def embed(
        ...
        for vnode in vnr.nodes:
            ...
            # 利用可能な中でCPUの多い順にソート
            candidates.sort(
                key=lambda sn: substrate.nodes[sn]["cpu"], reverse=True
            )
